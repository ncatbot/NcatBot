...
async def test_sysinfo_private_root_replies(tmp_path, monkeypatch):
    ...
    try:
        ...
        await h.settle(0.15)
        assert h.mock_api.called("post_private_array_msg")
        calls = h.mock_api.get_calls("post_private_array_msg")
        assert calls
...
async def test_sysinfo_denied_non_root(tmp_path, monkeypatch):
    ...
    try:
        ...
        await h.settle(0.15)
        assert h.mock_api.called("post_private_array_msg")
        text = extract_text(h.mock_api.get_calls("post_private_array_msg")[-1])
        assert "无权" in text
    finally:
        await h.stop()
...
async def test_sysinfo_denied_in_group_when_not_allowed(tmp_path, monkeypatch):
    ...
    try:
        ...
        await h.settle(0.15)
        assert h.mock_api.called("post_group_array_msg")
        text = extract_text(h.mock_api.get_calls("post_group_array_msg")[-1])
        assert "私聊" in text
    finally:
        await h.stop()
...
async def test_builtin_toggle_off_then_on(tmp_path, monkeypatch):
    ...
    try:
        ...
        await h.inject(private_message("!sysinfo", user_id="10001"))
        await h.settle(0.15)
        calls_off = h.mock_api.get_calls("post_private_array_msg")
        ...
        await h.settle(0.15)
        assert h.mock_api.called("post_private_array_msg")
        ok_text = extract_text(h.mock_api.get_calls("post_private_array_msg")[-1])
