...
class TestBasicPluginLoading:
    ...
    @pytest.mark.asyncio
    async def test_basic_plugin_loading_complete(self, test_suite: E2ETestSuite, basic_plugin_name):
        """测试插件基础加载功能（综合测试）
        ...
        # 5. 测试插件获取
        retrieved_plugin = test_suite.client.plugin_loader.get_plugin("basic_test_plugin")
...
class TestPluginLoadingWithCommands:
    ...
    @pytest.mark.asyncio
    async def test_command_plugin_loading_complete(self, test_suite: E2ETestSuite, command_plugin_name):
        """测试命令插件加载功能（综合测试）
...
class TestPluginLoadingWithConfig:
    ...
    @pytest.mark.asyncio
    async def test_config_plugin_loading_complete(self, test_suite: E2ETestSuite, config_plugin_name):
        """测试配置插件加载功能（综合测试）
...
class TestPluginLoadingWithHandlers:
    ...
    @pytest.mark.asyncio
    async def test_handler_plugin_loading_complete(self, test_suite: E2ETestSuite, handler_plugin_name):
        """测试事件处理器插件加载功能（综合测试）
        ...
        HandlerTestPlugin = test_suite.client.get_plugin_class_by_name("handler_test_plugin")
...
class TestFullFeaturePluginLoading:
    ...
    @pytest.mark.asyncio
    async def test_full_feature_plugin_loading_complete(self, test_suite: E2ETestSuite, full_feature_plugin_name):
        """测试完整功能插件加载（综合测试）
        ...
        FullFeaturePlugin = test_suite.client.get_plugin_class_by_name("full_feature_plugin")
...
class TestMultiplePluginLoading:
    ...
    async def test_multiple_plugins_loading_complete(
        ...
        # 3. 获取插件类
        HandlerTestPlugin = test_suite.client.get_plugin_class_by_name("handler_test_plugin")
        ConfigTestPlugin = test_suite.client.get_plugin_class_by_name("config_test_plugin")
