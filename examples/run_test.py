...
def ensure_project_root():
	...
	if not os.path.exists(marker):
		print("[WARN] 未在当前工作目录发现 pyproject.toml，建议在项目根目录运行：python -m examples.run_test", file=sys.stderr)
...
def main(argv: List[str] = None) -> int:
	ensure_project_root()
	parser = argparse.ArgumentParser(description="一键运行 UnifiedRegistry 文档验证示例")
	parser.add_argument(
		"--only",
		nargs="*",
	...
	if args.skip:
		...
		selected = [m for m in selected if not any(k in m.lower() or m == k for k in keys)]
	...
	print("=" * 72)
	print("汇总结果")
	print(f"总计: {len(results)}, 通过: {len(results) - failures}, 失败: {failures}, 用时: {elapsed_all:.2f}s")
	if failures:
		print("\n失败详情：")
		for res in results:
			if res["ok"]:
